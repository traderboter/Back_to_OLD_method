...
این سند توضیح می‌دهد که در **سیستم جدید ماژولار** وقتی داده‌های چهار تایم‌فریم (5m, 15m, 1h, 4h) برای تحلیل و ایجاد سیگنال معاملاتی دریافت می‌شوند، چه اتفاقاتی می‌افتد. در این سیستم، **سیگنال نهایی بر اساس امتیازدهی (Scoring) تولید می‌شود** که ترکیبی از تحلیل‌های مختلف است.
...
SignalOrchestrator (هماهنگ‌کننده اصلی)
    ↓
    ├─→ IndicatorOrchestrator (محاسبه اندیکاتورها)
...
#### 1. IndicatorOrchestrator (هماهنگ‌کننده اندیکاتورها)

**محل:** `signal_generation/analyzers/indicators/indicator_orchestrator.py:17`

نقش اصلی: مدیریت محاسبه، کش کردن و دسترسی به اندیکاتورها

**قابلیت‌ها:**
- محاسبه متمرکز 8 اندیکاتور اصلی (EMA, SMA, RSI, MACD, Bollinger Bands, Stochastic, ADX, ATR)
- Cache اندیکاتورها برای جلوگیری از محاسبات تکراری
- دسترسی واحد به اندیکاتورها برای تمام Analyzer ها
- استفاده از کلاس‌های مستقل BaseIndicator برای هر اندیکاتور

**مزایا:**
- ⚡ سرعت بالا (بدون محاسبات تکراری)
- 🎯 کد تمیز (تمام Analyzer ها از یک منبع استفاده می‌کنند)
- 🧪 قابلیت تست بالا (هر اندیکاتور مستقل است)
- 🔧 نگهداری آسان (تغییر در یک جا اعمال می‌شود)

#### 2. TimeframeScoreCache (کش امتیازات)

**محل:** `signal_generation/timeframe_score_cache.py:80`

نقش اصلی: جلوگیری از محاسبات تکراری برای یک تایم‌فریم

**نحوه کار:**
```python
# بررسی cache قبل از تحلیل
cached_result = self.timeframe_score_cache.get(symbol, timeframe, df)

if cached_result:
    # استفاده از نتیجه کش شده
    return cached_result
else:
    # محاسبه جدید
    result = await self._analyze_timeframe(...)
    # ذخیره در cache
    self.timeframe_score_cache.set(symbol, timeframe, df, result)
    return result
```

**شرایط invalidation (بی‌اعتبار شدن):**
- تغییر داده‌های بازار (کندل جدید)
- تغییر تنظیمات
- گذشت زمان معین

**مزایا:**
- 🚀 سرعت بسیار بالا در تحلیل مجدد
- 💾 کاهش مصرف CPU
- ⚖️ امکان تحلیل سریع‌تر چند نماد

#### 3. AnalysisContext (زمینه تحلیل مشترک)

داده‌ای که بین تمام Analyzer ها به اشتراک گذاشته می‌شود:

```python
@dataclass
class AnalysisContext:
    df: pd.DataFrame              # داده‌های OHLCV
    symbol: str                   # نماد (مثل BTCUSDT)
    timeframe: str               # تایم‌فریم (مثل 5m)
    indicators: Dict[str, Any]   # اندیکاتورهای محاسبه شده
    market_regime: Dict[str, Any]  # رژیم بازار
    config: Dict[str, Any]       # تنظیمات
    # ... سایر فیلدها
```

**مزایا:**
- همه Analyzer ها داده‌های یکسان دارند
- نیازی به pass کردن آرگومان‌های زیاد نیست
- اشتراک‌گذاری نتایج بین Analyzer ها

#### 4. SignalOrchestrator و روش‌های Multi-Timeframe Aggregation

**محل:** `signal_generation/orchestrator.py:854`

SignalOrchestrator هماهنگ‌کننده اصلی تولید سیگنال است و دو روش برای ترکیب امتیازات چند تایم‌فریمی دارد:

##### روش 1: Multi-TF Aggregation (OLD SYSTEM MODE) ⭐ پیش‌فرض

این روش تمام تایم‌فریم‌ها را تحلیل کرده و با وزن‌دهی ترکیب می‌کند:

```python
# تولید سیگنال برای هر تایم‌فریم
timeframe_signals = {}
for timeframe in ['5m', '15m', '1h', '4h']:
    signal, context = await generate_signal_with_context(symbol, timeframe)
    timeframe_signals[timeframe] = TimeframeSignal(...)

# ترکیب با وزن‌دهی
aggregated_signal = multi_tf_aggregator.aggregate_timeframe_scores(
    symbol, timeframe_signals
)
```

**مزایا:**
- ✅ استفاده از اطلاعات همه تایم‌فریم‌ها
- ✅ وزن‌دهی بر اساس اهمیت هر تایم‌فریم
- ✅ نتایج دقیق‌تر و پایدارتر
- ✅ سازگار با سیستم قدیمی

**کاربرد:** تحلیل جامع و دقیق (پیش‌فرض)

##### روش 2: Best Signal Selection (NEW SYSTEM MODE)

این روش ساده‌تر است و فقط بهترین سیگنال را انتخاب می‌کند:

```python
# تولید سیگنال برای هر تایم‌فریم
signals = []
for timeframe in ['5m', '15m', '1h', '4h']:
    signal = await generate_signal(symbol, timeframe)
...
**معایب:**
- ❌ اطلاعات سایر تایم‌فریم‌ها نادیده گرفته می‌شود

**کاربرد:** تحلیل سریع یا تست

**تنظیمات:**
```python
# config
use_multi_tf_aggregation = True  # روش 1 (پیش‌فرض)
use_multi_tf_aggregation = False # روش 2
```
...
**نکته مهم:** در سیستم جدید، تولید سیگنال توسط `SignalOrchestrator` انجام می‌شود که یک معماری ماژولار با 11 Analyzer مستقل دارد.
