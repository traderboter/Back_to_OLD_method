...
**وضعیت**: ✅ تأیید شده - در حال اجرا

---

## ✅ تصمیمات کلیدی (تأیید شده - 2025-01-20)

### 1. استراتژی پیاده‌سازی
- [x] **تصمیم**: **گزینه A** - مستقیماً در `signal_generation/` تغییر می‌دهیم
  - ✅ **انتخاب شده**: بله، مستقیماً تغییر می‌دهیم (سریع‌تر)
  - مزیت: سریع‌تر، بدون overhead merge
  - ریسک: باید دقت بیشتری در testing داشته باشیم
  - تصمیم: تمرکز بر TDD برای کاهش ریسک

### 2. استراتژی Testing
- [x] **تصمیم**: **گزینه A** - TDD، همزمان با کد
  - ✅ **انتخاب شده**: همزمان با هر مرحله
  - روش: Test-Driven Development (TDD)
  - مزیت: کد با کیفیت بالاتر، bug های کمتر، refactor ایمن
  - هزینه: کندتر (~30% زمان بیشتر)، اما بسیار امن‌تر

### 3. استراتژی Git
- [x] **تصمیم**: **گزینه A** - بعد از هر مرحله یک commit
  - ✅ **انتخاب شده**: بعد از هر مرحله commit
  - الگوی commit message: `feat: Add XYZ with tests`
  - مزیت: تاریخچه واضح، rollback آسان، review راحت‌تر
  - فرکانس: ~2-3 commit در روز

### 4. Configuration Management
- [x] **تصمیم**: **گزینه A** - Config فعلی را تغییر می‌دهیم
  - ✅ **انتخاب شده**: تغییر config فعلی
  - توجه: ممکن است breaking changes داشته باشد
  - راه‌حل: Documentation کامل برای migration
  - نکته: باید backward compatibility در کد رعایت شود

### 5. Validation Strategy
- [x] **تصمیم**: **گزینه C** - هر دو روش (مقایسه + Backtest)
  - ✅ **انتخاب شده**: مقایسه مستقیم + Backtest
  - مرحله 1: مقایسه output با سیستم قدیم (با همان input)
  - مرحله 2: Backtest performance و مقایسه نتایج
  - معیار موفقیت: تفاوت کمتر از ±5%
  - مزیت: اطمینان کامل از صحت پیاده‌سازی

### 6. Data for Testing
- [x] **تصمیم**: **گزینه B** - داده‌های ذخیره شده (CSV)
  - ✅ **انتخاب شده**: CSV های موجود
  - **مسیر داده‌ها**: `historical/BTC-USDT/`
  - **فایل‌های موجود**:
    - `5min.csv` (6.5MB، ~17,000 کندل)
    - `15min.csv` (2.4MB، ~6,000 کندل)
    - `1hour.csv` (747KB، ~1,500 کندل)
    - `4hour.csv` (340KB، ~400 کندل)
  - **فرمت**: `timestamp,open,high,low,close,volume,timestamp_unix`
  - **مزیت**: قابل تکرار، سریع، بدون نیاز به API
  - **استفاده**: برای هر دو unit tests و backtest
...
- [x] پاسخ دادن به 6 سوال کلیدی
- [x] تصمیم‌گیری درباره استراتژی
- [x] مستندسازی تصمیمات در این فایل

**✅ تکمیل شد**: تمام تصمیمات در بالا مستند شده است

#### Task 0.2: بررسی وضعیت Git ✅
```bash
# بررسی branch فعلی
git branch
# Output: claude/review-old-signal-docs-01TN5unho4YUF5Mn8h68eoeB

# بررسی status
git status
```

**✅ تصمیم**: مستقیماً در branch فعلی کار می‌کنیم (بدون ایجاد branch جدید)
**دلیل**: تغییرات به تدریج با test اعمال می‌شوند، ریسک پایین
