...
class CryptoTradingBot:
    ...
    def _load_config(self, config_path: str) -> Dict[str, Any]:
        ...
        try:
            ...
            # اعتبارسنجی عمیق تنظیمات
            try:
                from signal_generation.config_validator import validate_config
                logger.info("شروع اعتبارسنجی پیشرفته تنظیمات...")
                validate_config(config, strict=False)  # warnings را ignore می‌کند
                logger.info("✅ اعتبارسنجی پیشرفته تنظیمات موفق بود")
            except Exception as e:
                logger.error(f"⚠️  اعتبارسنجی تنظیمات با خطا مواجه شد: {e}")
                logger.warning("ادامه با تنظیمات فعلی (ممکن است مشکلاتی وجود داشته باشد)")
